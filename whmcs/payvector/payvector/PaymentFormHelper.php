	...
	public static function getTransactionResultFromPostVariables($aFormVariables, &$trTransactionResult, &$szHashDigest, &$szOutputMessage)
	{
		...
		try
		{
			...
			if(!$boErrorOccurred)
			{
				...
				if(isset($szCardType))
				{
					$trTransactionResult->setCardType($szCardType);
				}
				if(isset($szCardClass))
				{
					$trTransactionResult->setCardClass($szCardClass);
				}
				if(isset($szCardIssuer))
				{
					$trTransactionResult->setCardIssuer($szCardIssuer);
				}
				if(isset($nCardIssuerCountryCode))
				{
					$trTransactionResult->setCardIssuerCountryCode($nCardIssuerCountryCode);
				}
