...
class TransactionProcessor
{
	...
	public function getHostedPaymentForm(
		...
		$sessionHandler)
	{
		$sessionHandler->set( 'payvector_transaction_is_cross_reference', false, 'payvector');
		...
		$sessionHandler)
	{
		...
		$cdtCardDetailsTransaction->getCardDetails()->setCardName($cardOwner);
		...
		$sessionHandler)
	{
		...
		$sessionHandler->set( 'payvector_transaction_is_cross_reference', true, 'payvector');
...
abstract class DirectFinalTransactionResult extends BaseFinalTransactionResult
{
	...
	public function __construct($transactionProcessed, $toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		{			
			$sessionHandler->set( 'payvector_transaction_order_id', $this->getOrderID($sessionHandler), 'payvector');
		}
	}
	public function getTransactionMethod()
	{
	...
	{		
		return $this->todTransactionOutputData->getAmountReceived()->getValue();
	}
}
...
class CardDetailsFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function __construct($transactionProcessed,$toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		$sessionHandler->set( 'payvector_transaction_card_first_six', substr($toTransactionObject->getCardDetails()->getCardNumber(), 0, 6), 'payvector');
		$sessionHandler->set( 'payvector_transaction_card_last_four', substr($toTransactionObject->getCardDetails()->getCardNumber(), -4, 4) , 'payvector');
	}
...
class ThreeDSecureFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function getCardFirstSix($sessionHandler)
	{
  	  	$cardFirstSix = $sessionHandler->get('payvector_transaction_card_first_six', 'payvector');
		if (!empty($cardFirstSix)) 
		{
			$this->cardFirstSix = $sessionHandler->get('payvector_transaction_card_first_six', 'payvector');		
			$sessionHandler->delete('payvector_transaction_card_first_six', 'payvector');
		}
		return $this->cardFirstSix;
	}
	public function getCardLastFour($sessionHandler)
	{
		$cardLastFour = $sessionHandler->get('payvector_transaction_card_last_four', 'payvector');
		if (!empty($cardLastFour)) 
		{			
			$this->cardLastFour = $sessionHandler->get('payvector_transaction_card_last_four', 'payvector');			
			$sessionHandler->delete('payvector_transaction_card_last_four', 'payvector');
		}
		return $this->cardLastFour;
	}
	public function getOrderID($sessionHandler)
	{
		$orderID = $sessionHandler->get('payvector_transaction_order_id', 'payvector');
		if (!empty($orderID))  
		{
			$this->orderID = $sessionHandler->get('payvector_transaction_order_id', 'payvector');			
			$sessionHandler->delete('payvector_transaction_order_id', 'payvector');
		}
		return $this->orderID;
	}
}
...
class HostedPaymentFormFinalTransactionResult extends BaseFinalTransactionResult
{
	...
	public function getAmountReceived()
	{					
		return $this->trTransactionResult->getAmount();
	}
}
