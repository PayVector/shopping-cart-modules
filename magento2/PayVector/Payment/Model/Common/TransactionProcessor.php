...
class TransactionProcessor
{
	...
	public function getHostedPaymentForm(
		...
		$sessionHandler)
	{
		$sessionHandler->setData( 'payvector_transaction_is_cross_reference', false);
		...
		$sessionHandler)
	{
		...
		$sessionHandler->setData( 'payvector_transaction_is_cross_reference', true);
	...
	private function setSharedFields($toTransactionObject)
	{
		...
		if (intval($this->countryCode) > 0)
		{
			$toTransactionObject->getCustomerDetails()->getBillingAddress()->getCountryCode()->setValue(intval($this->countryCode));
		}
...
abstract class DirectFinalTransactionResult extends BaseFinalTransactionResult
{
	...
	public function __construct($transactionProcessed, $toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		{			
			$sessionHandler->setData( 'payvector_transaction_order_id', $this->getOrderID($sessionHandler));
		}
	}
	public function getTransactionMethod()
	{
...
class CardDetailsFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function __construct($transactionProcessed,$toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		$sessionHandler->setData( 'payvector_transaction_card_first_six', substr($toTransactionObject->getCardDetails()->getCardNumber(), 0, 6));
		$sessionHandler->setData( 'payvector_transaction_card_last_four', substr($toTransactionObject->getCardDetails()->getCardNumber(), -4, 4) );
	}
...
class ThreeDSecureFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function getCardFirstSix($sessionHandler)
	{
  	  	$cardFirstSix = $sessionHandler->getData('payvector_transaction_card_first_six');
		if (!empty($cardFirstSix)) 
		{
			$this->cardFirstSix = $sessionHandler->getData('payvector_transaction_card_first_six');		
	...
	public function getCardLastFour($sessionHandler)
	{
		$cardLastFour = $sessionHandler->getData('payvector_transaction_card_last_four');
		if (!empty($cardLastFour)) 
		{			
			$this->cardLastFour = $sessionHandler->getData('payvector_transaction_card_last_four');			
	...
	public function getOrderID($sessionHandler)
	{
		$orderID = $sessionHandler->getData('payvector_transaction_order_id');
		if (!empty($orderID))  
		{
			$this->orderID = $sessionHandler->getData('payvector_transaction_order_id');			
