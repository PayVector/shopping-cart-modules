...
class TransactionProcessor
{
	 ...
	 * @param  string $paymentFormDisplaysResult Whether the payment form displays the result to the user or passes it back to the merchant's system for display (optional)
	...
	{
		$_SESSION['payvector_transaction_is_cross_reference'] = false;
	...
	)
	{
		...
		$_SESSION['payvector_transaction_is_cross_reference'] = false;
		$cdtCardDetailsTransaction = new CardDetailsTransaction($this->rgeplRequestGatewayEntryPointList);
		...
		return new CardDetailsFinalTransactionResult($transactionProcessed, $cdtCardDetailsTransaction, $cdtrCardDetailsTransactionResult, $todTransactionOutputData);
	}
	...
	{
		$_SESSION['payvector_transaction_is_cross_reference'] = true;
		$crtCrossReferenceTransaction = new CrossReferenceTransaction($this->rgeplRequestGatewayEntryPointList);
		//set fields shared with a CardDetailsTransaction
		...
		return new CrossReferenceFinalTransactionResult($transactionProcessed, $crtCrossReferenceTransaction, $crtrCrossReferenceTransactionResult, $todTransactionOutputData);
	}
	...
	private function setSharedFields($toTransactionObject)
	{
		...
		$toTransactionObject->getCustomerDetails()->getBillingAddress()->getCountryCode()->setValue($this->countryCode);
	...
	public function check3DV2SecureResult(
		$crossReference,
		$ThreeDSMethodData
	)
	{
		$tdseThreeDSecureEnvironment = new ThreeDSecureEnvironment($this->rgeplRequestGatewayEntryPointList);
		$tdseThreeDSecureEnvironment->getMerchantAuthentication()->setMerchantID($this->merchantID);
		$tdseThreeDSecureEnvironment->getMerchantAuthentication()->setPassword($this->merchantPassword);
		...
		return $tdsarThreeDSecureAuthenticationResult;
		//return new ThreeDSecureFinalTransactionResult($transactionProcessed, $tdsaThreeDSecureAuthentication, $tdsarThreeDSecureAuthenticationResult, $todTransactionOutputData);
	}
	...
	{
		$tdsaThreeDSecureAuthentication = new ThreeDSecureAuthentication($this->rgeplRequestGatewayEntryPointList);
...
interface FinalTransactionResult
{
	...
	/**
	 * The last four digits of the card number (if available). Only applicable for the Direct API integration method
	 * @return string|null
	 */
...
abstract class DirectFinalTransactionResult extends BaseFinalTransactionResult
{
	 ...
	 */
	public function __construct($transactionProcessed, $toTransactionObject, $troTransactionResultObject, $todTransactionOutputData)
	{
		...
		{
			$_SESSION['payvector_transaction_order_id'] = $this->getOrderID();
		}
	}
	public function getTransactionMethod()
	{
	...
	{		
		...
		if (isset($this->troTransactionResultObject) && method_exists($this->troTransactionResultObject, 'getPreviousTransactionResult')) {
...
class CardDetailsFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function __construct($transactionProcessed,$toTransactionObject, $troTransactionResultObject, $todTransactionOutputData)
	{
		parent::__construct($transactionProcessed, $toTransactionObject, $troTransactionResultObject, $todTransactionOutputData);

		$_SESSION['payvector_transaction_card_last_four'] = substr($toTransactionObject->getCardDetails()->getCardNumber(), -4, 4);
	}
...
class ThreeDSecureFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function getCardLastFour()
	{
		if(isset($_SESSION['payvector_transaction_card_last_four']))
		{
			$this->cardLastFour = $_SESSION['payvector_transaction_card_last_four'];
			unset($_SESSION['payvector_transaction_card_last_four']);
		}
		return $this->cardLastFour;
	}
	public function getOrderID()
	{
		if(isset($_SESSION['payvector_transaction_order_id']))
		{
			$this->orderID = $_SESSION['payvector_transaction_order_id'];
			unset($_SESSION['payvector_transaction_order_id']);
		}
		return $this->orderID;
	}
}
