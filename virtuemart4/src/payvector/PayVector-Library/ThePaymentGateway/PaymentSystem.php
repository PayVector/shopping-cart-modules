<?php	
	...
	class RequestGatewayEntryPointList
	{
		...
		{
			if (class_exists($ComparerClassName) && method_exists($ComparerClassName, $ComparerMethodName)) {
				usort($this->m_lrgepRequestGatewayEntryPoint, [$ComparerClassName, $ComparerMethodName]);
			}	
		}
	...
	class ThreeDSecureBrowserDetails
	{
	    ...
	    public function __construct()
	    {
	    	...
	    	$this->m_szUserAgent = "";
			$this->m_szLanguage = "";
			$this->m_nScreenHeight = new NullableInt();
			$this->m_nScreenWidth = new NullableInt();
			$this->m_nScreenColourDepth = new NullableInt();
			$this->m_nTimeZone = new NullableInt();
			$this->m_boJavaEnabled = new NullableBool();
			$this->m_boJavaScriptEnabled = new NullableBool();
	    }
	}
	   	...
	   	public function processTransaction(CardDetailsTransactionResult &$cdtrCardDetailsTransactionResult = null, TransactionOutputData &$todTransactionOutputData = null)
	   	{
	       	...
	       	if ($this->m_tdTransactionDetails != null)
	        {
	          	...
	          	if ($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails() != null)
	            {
					...
					if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails()->getLanguage()))
	                {
					...
					if ($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails()->getJavaEnabled() != null)
	                {
	                    ...
	                    {
	                    	$sSOAPClient->addParam("PaymentMessage.TransactionDetails.ThreeDSecureBrowserDetails.JavaEnabled", SharedFunctions::boolToString($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails()->getJavaEnabled()->getValue()));
	                    }
	                }
					if ($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails()->getJavaScriptEnabled() != null)
	                {
	                    ...
	                    {
	                    	$sSOAPClient->addParam("PaymentMessage.TransactionDetails.ThreeDSecureBrowserDetails.JavaScriptEnabled", SharedFunctions::boolToString($this->m_tdTransactionDetails->getThreeDSecureBrowserDetails()->getJavaScriptEnabled()->getValue()));
	                    }
	                }
	            }
				if ($this->m_tdTransactionDetails->getThreeDSecurePassthroughData() != null)
                {
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getEnrolmentStatus()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getAuthenticationStatus()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getECI()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getAuthenticationValue()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getTransactionIdentifier()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecurePassthroughData()->getThreeDSecureVersion()))
                    {
                ...
                {
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureNotificationDetails()->getFingerprintNotificationURL()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureNotificationDetails()->getChallengeNotificationURL()))
                    {
                ...
                {
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureAuthenticationExemption()->getExemptionType()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureAuthenticationExemption()->getExemptionReason()))
                    {
                    ...
                    if (!SharedFunctions::isStringNullOrEmpty($this->m_tdTransactionDetails->getThreeDSecureAuthenticationExemption()->getExemptionIndicator()))
                    {
			...
			if ($boTransactionSubmitted)
			{
				$cdtrCardDetailsTransactionResult = PvSharedFunctions::getPaymentMessageGatewayOutput($sSOAPClient->getXmlTag(), "CardDetailsTransactionResult", $goGatewayOutput);
	
				$todTransactionOutputData = PvSharedFunctions::getTransactionOutputData($sSOAPClient->getXmlTag(), $lgepGatewayEntryPoints);
			}
	...
	class CrossReferenceTransaction extends GatewayTransaction 
	{
	    ...
	    public function processTransaction(CrossReferenceTransactionResult &$crtrCrossReferenceTransactionResult = null, TransactionOutputData &$todTransactionOutputData = null)
	    {
	        ...
	        $boTransactionSubmitted = GatewayTransaction::processTransactionBase($sSOAPClient, "PaymentMessage", "CrossReferenceTransactionResult", "TransactionOutputData", $goGatewayOutput, $lgepGatewayEntryPoints);
	       	...
	       	if ($boTransactionSubmitted)
	        {
				$crtrCrossReferenceTransactionResult = PvSharedFunctions::getPaymentMessageGatewayOutput($sSOAPClient->getXmlTag(), "CrossReferenceTransactionResult", $goGatewayOutput);
	...
	class ThreeDSecureEnvironment extends GatewayTransaction
	{
		...
		public function processTransaction(ThreeDSecureEnvironmentResult &$tdserThreeDSecureEnvironmentResult = null, TransactionOutputData &$todTransactionOutputData = null)
		{
	        ...
	        $goGatewayOutput = null;
	       	...
	       	$sSOAPClient = new SOAP("ThreeDSecureEnvironment", GatewayTransaction::getSOAPNamespace());
	       	if ($this->m_tdsedThreeDSecureEnvironmentData != null)
	        {
	        	if (!SharedFunctions::isStringNullOrEmpty($this->m_tdsedThreeDSecureEnvironmentData->getCrossReference()))
	            {
	            ...
	            if (!SharedFunctions::isStringNullOrEmpty($this->m_tdsedThreeDSecureEnvironmentData->getMethodData()))
	            {
	        ...
	        if ($boTransactionSubmitted)
	      	{
				$tdserThreeDSecureEnvironmentResult = PvSharedFunctions::getPaymentMessageGatewayOutput($sSOAPClient->getXmlTag(), "ThreeDSecureEnvironmentResult", $goGatewayOutput);
	...
	class ThreeDSecureAuthentication extends GatewayTransaction
	{
		...
		public function processTransaction(ThreeDSecureAuthenticationResult &$tdsarThreeDSecureAuthenticationResult = null, TransactionOutputData &$todTransactionOutputData = null)
		{
	       	...
	       	if ($this->m_tdsidThreeDSecureInputData != null)
	        {
	            ...
	            if (!SharedFunctions::isStringNullOrEmpty($this->m_tdsidThreeDSecureInputData->getMethodData()))
	            {
				...
				if (!SharedFunctions::isStringNullOrEmpty($this->m_tdsidThreeDSecureInputData->getCRES()))
	            {
	        ...
	        if ($boTransactionSubmitted)
	      	{
				$tdsarThreeDSecureAuthenticationResult = PvSharedFunctions::getPaymentMessageGatewayOutput($sSOAPClient->getXmlTag(), "ThreeDSecureAuthenticationResult", $goGatewayOutput);
	...
	class PvSharedFunctions
	{
		public static function getTransactionOutputData(XmlTag $xtTransactionOutputDataXmlTag, GatewayEntryPointList $lgepGatewayEntryPoints = null)
		{
            ...
            $ctdCardTypeData = PvSharedFunctions::getCardTypeData($xtTransactionOutputDataXmlTag, "TransactionOutputData.CardTypeData");
			...
			if (!$xtTransactionOutputDataXmlTag->getStringValue("TransactionOutputData.ThreeDSecureOutputData.ThreeDSecureVersion", $szThreeDSecureVersion) ||
				SharedFunctions::isStringNullOrEmpty($szThreeDSecureVersion))
            {
