...
abstract class DirectFinalTransactionResult extends BaseFinalTransactionResult
{
	...
	public function __construct($transactionProcessed, $toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		{			
			$sessionHandler['payvector_transaction_order_id'] =  $this->getOrderID($sessionHandler);
		}
	}
	public function getTransactionMethod()
	{
...
class CardDetailsFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function __construct($transactionProcessed,$toTransactionObject, $troTransactionResultObject, $todTransactionOutputData, $sessionHandler)
	{
		...
		$sessionHandler['payvector_transaction_card_first_six'] = substr($toTransactionObject->getCardDetails()->getCardNumber(), 0, 6);
		$sessionHandler['payvector_transaction_card_last_four'] =  substr($toTransactionObject->getCardDetails()->getCardNumber(), -4, 4) ;
	}
...
class ThreeDSecureFinalTransactionResult extends DirectFinalTransactionResult
{
	...
	public function getCardFirstSix($sessionHandler)
	{
		if (isset($sessionHandler['payvector_transaction_card_first_six']) && !empty($sessionHandler['payvector_transaction_card_first_six'])) 
		{
			$this->cardFirstSix = $sessionHandler['payvector_transaction_card_first_six'];		
			unset($sessionHandler['payvector_transaction_card_first_six']);
		}
		return $this->cardFirstSix;
	}
	public function getCardLastFour($sessionHandler)
	{
		if (isset($sessionHandler['payvector_transaction_card_last_four']) && !empty($sessionHandler['payvector_transaction_card_last_four'])) 
		{			
			$this->cardLastFour = $sessionHandler['payvector_transaction_card_last_four'];			
			unset($sessionHandler['payvector_transaction_card_last_four']);
		}
		return $this->cardLastFour;
	}
	public function getOrderID($sessionHandler)
	{
		if (isset($sessionHandler['payvector_transaction_order_id']) && !empty($sessionHandler['payvector_transaction_order_id']))  
		{
			$this->orderID = $sessionHandler['payvector_transaction_order_id'];			
			unset($sessionHandler['payvector_transaction_order_id']);
		}
		return $this->orderID;
	}
}
