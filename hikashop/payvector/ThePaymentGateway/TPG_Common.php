<?php
	namespace net\thepaymentgateway\common;
	//XMLEntities
	$g_XMLEntities = array();
	...
	$GLOBALS['g_XMLEntities'] = $g_XMLEntities;
	...
	{		
		...
		public static function replaceCharsInStringWithEntities($szString)
		{
			...
			$xmlEntities = $GLOBALS['g_XMLEntities']; // now this should be the array
			...
			$szReturnString;
	      	...
	      	for ($nCount = 0; $nCount < strlen($szString); $nCount++)
	      	{
	      		$boFound = false;
	           	...
	           	while (!$boFound && 
	                  	$nHTMLEntityCount < count($xmlEntities))
	            {
	            	//$test1 = htmlspecialchars('&');
	                  		
	                if ($xmlEntities[$nHTMLEntityCount]->getReplacement() == htmlspecialchars($szString[$nCount]))
	                {
	                	$boFound = true;
	                }
	                else 
	        	...
	        	if ($boFound)
	        	{
	        		$szReturnString .= $xmlEntities[$nHTMLEntityCount]->getReplacement();
	        	}
	        	else 
	        	{
	            ...
	            if ($boFound)
	           	{
	                ...
	                while (!$boFoundEntity &&
	                      	$nXMLEntityCount < count($xmlEntities))
	              	{
	                	if (strtoupper($xmlEntities[$nXMLEntityCount]->getReplacement()) == strtoupper($szAmpersandBuffer))
	                    {
	                    	$boFoundEntity = true;
	                    }
	                    else
	                ...
	                if ($boFoundEntity)
	              	{
	                	$szReturnString .= $xmlEntities[$nXMLEntityCount]->getCharCode();
	                }
	                else
	                {
	                 	$szReturnString .= $szAmpersandBuffer;
